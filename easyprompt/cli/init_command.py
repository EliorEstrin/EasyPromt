...
def configure_llm_providers() -> Dict[str, Any]:
    ...
    while True:
        ...
        # Show current status if any providers configured
        if config:
            console.print(f"\n[green]✅ Configured providers: {len([k for k in config.keys() if 'API_KEY' in k])}[/green]")

        choice = Prompt.ask(
            "\nChoose provider to configure",
            choices=[p[0] for p in providers],
            default="done" if config else "gemini"
        )

        if choice == "gemini":
            result = configure_gemini()
            if result:  # Only update if user actually provided a key
                config.update(result)
                console.print("[green]✅ Google Gemini configured![/green]")
        elif choice == "openai":
            result = configure_openai()
            if result:  # Only update if user actually provided a key
                config.update(result)
                console.print("[green]✅ OpenAI configured![/green]")
        elif choice == "anthropic":
            result = configure_anthropic()
            if result:  # Only update if user actually provided a key
                config.update(result)
        ...
        # Small pause to show the success message
        if choice != "done":
            console.input("\n[dim]Press Enter to continue...[/dim]")
...
def configure_gemini() -> Dict[str, Any]:
    ...
    if not api_key.strip():
        console.print("[yellow]No API key provided, skipping Gemini configuration[/yellow]")
        return {}
...
def configure_openai() -> Dict[str, Any]:
    ...
    if not api_key.strip():
        console.print("[yellow]No API key provided, skipping OpenAI configuration[/yellow]")
        return {}
...
def configure_anthropic() -> Dict[str, Any]:
    ...
    if not api_key.strip():
        console.print("[yellow]No API key provided, skipping Anthropic configuration[/yellow]")
        return {}
...
def write_env_file(env_file: Path, config: dict):
    """Write configuration to .env file with all options."""
    with open(env_file, "w") as f:
        ...
        all_settings = {
            ...
            "LLM Providers": {
                "GEMINI_API_KEY": (config.get("GEMINI_API_KEY", ""), "Google Gemini API key"),
                "OPENAI_API_KEY": (config.get("OPENAI_API_KEY", ""), "OpenAI API key"),
                "ANTHROPIC_API_KEY": (config.get("ANTHROPIC_API_KEY", ""), "Anthropic Claude API key")
            },
            "Pinecone Settings": {
